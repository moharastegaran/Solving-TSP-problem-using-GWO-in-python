...
from data_io import read, generate
from CostFunction_TSP import CostFunction
...
"""
We may have only one pair of boundary for every dim or separate boundaries
For each dim, so for the ease of use, we cast numeric boundaries to 1d array
"""
...
Then we take into accounts these values and update them with their more accurate values
"""
...
for l in range(0, Max_iter):
    for i in range(0, SearchAgents_no):
        ...
        for j in range(dim):
            ...
            # Calculate cost function for each search agent, [i,:]
            # DIFF
            _cities = []
            poses = numpy.argsort(Positions[i,:])
            _cities[:] = [cities_list[k] for k in poses]
            fitness = CostFunction(_cities).route_distance()
        ...
        for i in range(0, SearchAgents_no):
            ...
            PlotSolution(np.argsort(Alpha_pos), cities_list).plot() #DIFF
